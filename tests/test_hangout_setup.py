...
def _draft_with_invites(db, *names):
    hangout = Hangout(status=HangoutStatus.draft, motive="Beach day")
    db.add(hangout)
    db.flush()
    profiles = []
    for index, name in enumerate(names):
        profile = Profile(name=name, phone=f"+1555111{index:04d}")
        db.add(profile)
        db.flush()
...
def test_setup_sends_to_the_selected_profiles(db):
    ...
    setup_hangout(db, load_hangout(db, hangout.id), [sam.id, lee.id])

    invites = load_hangout(db, hangout.id).invites
...
def test_deselected_uninvited_rows_are_dropped(db):
    ...
    setup_hangout(db, load_hangout(db, hangout.id), [sam.id])
...
def test_already_messaged_invites_survive_a_narrower_selection(db):
    ...
    invites = {inv.profile_id: inv for inv in load_hangout(db, hangout.id).invites}
